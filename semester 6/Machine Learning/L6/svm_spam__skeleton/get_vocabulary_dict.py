...
def get_vocabulary_dict(dict_path) -> Dict[int, str]:
    ...
    vocab_to_array = np.loadtxt(dict_path, dtype="str")
    vocab_keys = list(map(int, vocab_to_array[:,0].tolist()))
    vocab_values = vocab_to_array[:,1].tolist()

    vocabulary_dict = {}
    for i in range(len(vocab_values)):
         vocabulary_dict[vocab_keys[i]] = vocab_values[i]
